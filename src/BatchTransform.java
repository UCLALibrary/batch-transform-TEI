			...
			if (xmlFiles != null) {
				for (File xmlFile : xmlFiles) {
					...
					Transformer transformer = factory.newTransformer(new StreamSource(
							"/Users/parinitamulak/eclipse-workspace/BatchTransformTEI/src/xslt/TEIManuscriptContents.xsl"));
					System.out.println("what is filename:" + xmlFile.getName());
					System.out.println(
							"full encode file with parent path: " + xmlFile.getParent() + '/' + encodedFileName);
					// Perform the transformation
					if (xmlFile.getName().indexOf("ark%3A21198%2F") == 0) {
						transformer.transform(new StreamSource(xmlFile.getParent() + '/' + encodedFileName),
								new StreamResult(new FileOutputStream(outputFilePath)));
					} else {
						transformer.transform(new StreamSource(inputFilePath),
								new StreamResult(new FileOutputStream(outputFilePath)));
					}
